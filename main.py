...
def get_operation(input_string: str, operators: typing.Iterable, priorities: dict[str, int], parenthesis: bool = False) -> ExpressionTree | None:
    if input_string == "":
        print(">> Entrée vide")
        return None
    ...
    for index in range(max(priorities.values()), -1, -1):# de 2 à 0 généralement
        for i, part in enumerate(expression_parts):# expression_parts(5+5) -> (0, '5'), (1, '+'), (2, '5')
            if part in operators and 0 < i < len(expression_parts) - 1 and priorities[part] == index:
                ...
                expression_parts[i - 1], expression_parts[i], expression_parts[i + 1] = f"@{index}{j}", None, f"@{index}{j}"# ex[i-1]=5, ex[i]=+, ex[i+1]=5    
            
            elif part not in operators and part is not None and part[0] != "@": #tkt j'avis pas envie de rajouter une couche de if
                try: float(part)
                except ValueError:
    ...
    expressions_dict: dict[int, list[ExpressionTree]] = {i:[] for i in range(max(priorities.values()))}
    ...
    for i in range(max(priorities.values()) + 1)[::-1]:
        for j, op in enumerate(string_operations[i]):
            ...
            if op.first[0] == "@" and op.second[0] == "@":
                expressions_dict[i].append(ExpressionTree(
                    get_value(string_operations, expressions_dict, op.first),
                    op.operator,
                    get_value(string_operations, expressions_dict, op.second)
                ))
                set_value(string_operations, op.first, (str(i), "@", str(j)))
                set_value(string_operations, op.second, (str(i), "@", str(j)))

            elif op.first[0] == "@":
                expressions_dict[i].append(ExpressionTree(
                    get_value(string_operations, expressions_dict, op.first),
                    op.operator,
                    Term(float(op.second))
                ))
                set_value(string_operations, op.first, (str(i), "@", str(j)))
    ...
    final_expression = expressions_dict[min(priorities.values())][0]
    final_expression.set_parenthesis(True)
    return final_expression

def get_value(string_operations: dict[int, list[StringOperation]], expressions_dict: dict[int, list[ExpressionTree]], str_ptr: str) -> ExpressionTree:
    if str_ptr[0] != "@":
    ...
    return ExpressionTree(Term(float(str_op.first)), str_op.operator, Term(float(str_op.second)))


def set_value(string_operations: dict[int, list[StringOperation]], str_ptr: str, value: tuple[str]):
    if str_ptr[0] != "@":
        raise ValueError(f"\"{str_ptr}\" does not contain @ at the beginning")
    elif len(str_ptr) != 3:
        raise ValueError(f"\"{str_ptr}\" has not a lenght of 3")
